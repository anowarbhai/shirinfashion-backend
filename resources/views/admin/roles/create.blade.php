...
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-lg shadow-sm p-6">
        <form action="{{ route('admin.roles.store') }}" method="POST" class="space-y-6">
                ...
                <div class="max-h-80 overflow-y-auto border border-gray-200 rounded-lg p-4 space-y-4">
                    ...
                    <div class="border-b border-gray-100 pb-3 last:border-0">
                        <div class="flex items-center justify-between mb-2">
                            ...
                            <label class="flex items-center space-x-1 cursor-pointer text-sm text-rose-600 hover:text-rose-800">
                                <input type="checkbox" class="check-all-permissions rounded text-rose-500 focus:ring-rose-500" data-group="{{ $group }}">
                                <span class="font-medium">Check All</span>
                            </label>
                        </div>
                        <div class="grid grid-cols-2 gap-2">
                            @foreach($perms as $permission)
                            <label class="flex items-center space-x-2 cursor-pointer permission-checkbox permission-group-{{ $group }}">
            ...
            <script>
                document.querySelectorAll('.check-all-permissions').forEach(function(checkbox) {
                    checkbox.addEventListener('change', function() {
                        const group = this.dataset.group;
                        const checkboxes = document.querySelectorAll('.permission-group-' + group + ' input[type="checkbox"]');
