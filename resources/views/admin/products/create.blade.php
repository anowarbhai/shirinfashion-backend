...
@section('title', 'Add Product - Shirin Fashion Admin')
...
<div class="bg-white rounded-xl shadow-sm p-6">
    <form method="POST" action="{{ route('admin.products.store') }}" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div id="stock_quantity_field" class="{{ old('manage_stock') ? '' : 'hidden' }}">
                <label class="block text-sm font-medium text-gray-700 mb-2">Stock Quantity</label>
                <input type="number" name="stock_quantity" value="{{ old('stock_quantity', 0) }}"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-rose-500">
            </div>

            <div id="stock_status_field" class="{{ old('manage_stock') ? 'hidden' : '' }}">
        ...
        <div class="mt-6">
            ...
            <div id="thumbnail-preview" class="mt-2 {{ old('image') ? '' : 'hidden' }}">
                <img src="{{ old('image') }}" alt="Thumbnail" class="w-32 h-32 object-cover rounded">
        ...
        <div class="mt-6">
            ...
            <div id="gallery-preview" class="mt-2 flex flex-wrap gap-2">
                @if(old('images'))
                    @foreach(json_decode(old('images')) as $img)
                        @php
                            $imgSrc = $img;
        ...
        <div class="mt-6 grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="flex items-center">
                <input type="checkbox" name="step_url_enabled" value="1" {{ old('step_url_enabled') ? 'checked' : '' }}
                    class="w-4 h-4 text-rose-600 border-gray-300 rounded">
        ...
        <div class="mt-6">
            ...
            <div class="flex flex-wrap gap-2">
                @foreach($tags as $tag)
                    <label class="inline-flex items-center">
                        <input type="checkbox" name="tag_ids[]" value="{{ $tag->id }}" {{ in_array($tag->id, old('tag_ids', [])) ? 'checked' : '' }}
                            class="w-4 h-4 text-rose-600 border-gray-300 rounded">
                ...
                <div class="mb-3">
                    ...
                    <div class="flex flex-wrap gap-2 mt-1">
                        @foreach($attribute->values as $value)
                            <label class="inline-flex items-center">
                                <input type="checkbox" name="attribute_value_ids[]" value="{{ $value->id }}" {{ in_array($value->id, old('attribute_value_ids', [])) ? 'checked' : '' }}
                                    class="w-4 h-4 text-rose-600 border-gray-300 rounded">
...
<script>
    ...
    submitBtn.addEventListener('click', function(e) {
        ...
        document.getElementById('short_description').value = shortDescVal;
