    ...
    <form method="POST" action="{{ route('admin.coupons.update', $coupon) }}">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="number" name="discount_value" value="{{ old('discount_value', $coupon->discount_value) }}" required min="0" step="0.01"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-rose-500"
                    placeholder="0.00">
            </div>
            ...
            <div>
                ...
                <input type="number" name="max_discount_amount" value="{{ old('max_discount_amount', $coupon->max_discount_amount) }}" min="0" step="0.01"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-rose-500"
                    placeholder="0.00">
            </div>
            ...
            <div>
                ...
                <input type="date" name="expire_date" value="{{ old('expire_date', $coupon->expire_date?->format('Y-m-d')) }}"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-rose-500">
            </div>
            ...
            <div class="flex items-center gap-6 mt-6">
                ...
                <label class="flex items-center">
                    <input type="checkbox" name="is_featured" value="1" {{ old('is_featured', $coupon->is_featured) ? 'checked' : '' }}
                        class="w-4 h-4 text-rose-600 border-gray-300 rounded">
        ...
        <div class="mt-6 p-4 bg-gray-50 rounded-lg">
            <h4 class="font-medium text-gray-700 mb-2">Usage Statistics</h4>
            <p class="text-sm text-gray-600">Total times used: {{ $coupon->usage_count }}</p>
        </div>
